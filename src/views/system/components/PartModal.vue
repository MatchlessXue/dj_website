    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item
          ...
          label='角色名称'
          hasFeedback >
          <a-input disabled v-model="tmpRecord.roleName" />
        </a-form-item>
        <!-- <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
...
import { getPartListUser } from '@/api/account.js'
  ...
  methods: {
    ...
    edit (record) {
      ...
      for (const item of record.departmentList) {
        departments.push({ value: item.departmentId, label: item.departmentName })
      }
      this.$nextTick(() => {
    ...
    handleOk () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          let tempDepartments = []
          for (let item of values.departments) {
            tempDepartments.push(item.value)
          }
          let tmpValues = {}
          tmpValues.departments = tempDepartments
